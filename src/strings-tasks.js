...
function isPalindrome(str) {
  const tempStr = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  if (tempStr.length === 0) return false;
  for (let i = 0; i < tempStr.length / 2; i += 1) {
    if (
      tempStr[i].toLowerCase() !== tempStr[tempStr.length - i - 1].toLowerCase()
    )
      return false;
  }
  return true;
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    newStr[i] = words[i].split('').reverse().join('');
  }
  return newStr.join(' ');
}
...
function invertCase(str) {
  const newStr = [];
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] >= 'a' && str[i] <= 'z') newStr[i] = str[i].toUpperCase();
    else if (str[i] >= 'A' && str[i] <= 'Z') newStr[i] = str[i].toLowerCase();
    else newStr[i] = str[i];
  }
  return newStr.join('');
}
...
function getCardId(value) {
  let temp = 0;
  const card1 = value.slice(-1);
  const card2 = value.split(card1)[0];
  switch (card1) {
    case '♦':
      temp = 13;
      break;
  ...
  switch (card2) {
    ...
    case 'K':
      return temp + 12;
    default:
      return parseInt(card2, 10) + temp - 1;
  }
}
